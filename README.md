...
```shell
python Tansen/do_tts.py --text "I'm going to speak this" --voice random --preset fast
```
...
Tansen can be used programmatically, like so:
```python
...
Tansen was specifically trained to be a multi-speaker model. It accomplishes this by consulting reference clips.
...
To add new voices to Tansen, you will need to do the following:
...
6. Run Tansen utilities with --voice=<your_subdirectory_name>.

### Picking good reference clips

As mentioned above, your reference clips have a profound impact on the output of Tansen. Following are some tips for picking
good clips:

1. Avoid clips with background music, noise or reverb. These clips were removed from the training dataset. Tansen is unlikely to do well with them.
...
Tansen ingests reference clips by feeding them through individually through a small submodel that produces a point latent,
...
This lends itself to some neat tricks. For example, you can combine feed two different voices to Tansen and it will output
...
```commandline
python Tansen/is_this_from_Tansen.py --clip=<path_to_suspicious_audio_file>
```
...
Tansen TTS is inspired by OpenAI's DALLE, applied to speech data and using a better decoder. It is made up of 5 separate
models that work together. I've assembled a write-up of the system architecture here:
[https://nonint.com/2022/04/25/Tansen-architectural-design-doc/](https://nonint.com/2022/04/25/Tansen-architectural-design-doc/)
...
Tansen v2 works considerably better than I had planned. When I began hearing some of the outputs of the last few versions, I began
...
4. I am releasing a separate classifier model which will tell you whether a given audio clip was generated by Tansen or not. See `Tansen-detect` above.
